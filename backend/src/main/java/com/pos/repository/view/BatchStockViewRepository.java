...
public interface BatchStockViewRepository extends JpaRepository<BatchStockView, Long> {
    ...
    /**
     * Các lô còn tồn TRÊN KỆ của 1 sản phẩm, FEFO theo HSD — dùng để bán (POS chỉ bán hàng trên kệ).
    ...
    @Query("""
            ...
            """)
    List<BatchStockView> findAvailableBatchesFefo(@Param("productId") Long productId,
                                                  @Param("storeId") Long storeId);

    /** Các lô còn tồn TRONG KHO của 1 sản phẩm tại 1 CHI NHÁNH, FEFO theo HSD — dùng để chọn lô đưa lên kệ. */
    @Query("""
            SELECT v FROM BatchStockView v
            WHERE v.productId = :productId AND v.storeId = :storeId AND v.inWarehouse > 0
            ORDER BY CASE WHEN v.expiryDate IS NULL THEN 1 ELSE 0 END, v.expiryDate, v.batchId
            """)
    List<BatchStockView> findWarehouseBatchesFefo(@Param("productId") Long productId,
                                                  @Param("storeId") Long storeId);

    /** Tất cả lô CÒN HÀNG của 1 sản phẩm tại 1 chi nhánh (FEFO theo HSD) — xem chi tiết kho/kệ theo lô. */
    @Query("""
            SELECT v FROM BatchStockView v
