  ...
  return (
    <div>
      ...
      <InfoBanner id="shelfmanage" title="Kệ trưng bày">
        ...
        từ <b>lô nào</b>, <b>HSD</b> bao nhiêu. Việc đưa hàng từ kho lên kệ làm ở trang <b>Lên kệ</b>.
      </InfoBanner>
      ...
      <ShelfContentModal shelf={detail} onHide={() => setDetail(null)} />
...
/** Modal xem kệ đang chứa sản phẩm gì, lô nào, HSD. */
function ShelfContentModal({ shelf, onHide }) {
  const toast = useToast()
  ...
  return (
    <Modal show={!!shelf} onHide={onHide} centered size="lg">
      <Modal.Header closeButton><Modal.Title>Kệ {shelf.code}{shelf.name ? ` · ${shelf.name}` : ''}</Modal.Title></Modal.Header>
      <Modal.Body>
        {loading ? <Loading /> : (
          <Table size="sm" hover className="mb-0">
            <thead><tr><th>Sản phẩm</th><th>HSD (lô)</th><th className="text-end">Số lượng trên kệ</th></tr></thead>
            <tbody>
              {items.map((it) => (
                <tr key={it.batchId}>
              ...
              {items.length === 0 && <tr><td colSpan={3}><EmptyState icon="bi-inboxes" title="Kệ đang trống" /></td></tr>}
            </tbody>
          </Table>
        )}
      </Modal.Body>
